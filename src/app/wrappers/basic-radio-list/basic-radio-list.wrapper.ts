...
import { BaseConfig, BaseWrapperComponent } from '../../core/base/base.wrapper';
...
export class BasicRadioListWrapper
  extends BaseWrapperComponent<RadioListConfig>
  implements OnInit
{
  ...
  protected override initializeForm(): void {
    this.form = this.fb.group({
      selectedOption: [null, this.buildValidators()],
    });
  }

  protected override buildValidators(): ValidatorFn[] {
    const validators: ValidatorFn[] = [];

    if (this.config?.validations?.required) {
      validators.push(Validators.required);
    }

    if (this.config?.validations?.custom) {
      validators.push((control: any) => {
        const isValid = this.config.validations?.custom?.(control.value);
  ...
  protected override buildErrorMessages(): Record<string, string> {
    return {
